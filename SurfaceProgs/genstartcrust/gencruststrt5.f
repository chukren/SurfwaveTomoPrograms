c  gencruststrt5.f 
c  c  
c  same as gencruststrt3.f, except modified to use laterally varying starting
c  mantle model from Brandon's body wave inversion and crustal model from a 
c  previous phase velocity inversion - i.e. merge shallow structure with uniform
c  mantle structure, then perturb mantle structure with Brandon's model
c
...
c  assume start in lower left (SW) corner
      ...
      character*70  mohofile, defaultfile, rayl8secfile, Brandonfile
      character*70 priorcrustfile
      ...
      open(18, file = Brandonfile)
      open(22, file = priorcrustfile)
      
      write(11,*) beglat,endlat,latinc, beglon,endlon, loninc
      write(21,*) beglat,endlat,latinc, beglon,endlon, loninc
...
300   format (2F9.3,1x,f4.1)
	enddo
      ...
      close(unit=12)
      close(unit=17)
c  Now alter mantle part of model to incorporate laterally varying velocities
c  from Brandon's body wave inversion
      open(11, file = 'tempoutput')
      call mantlevar
      close (unit=21)
      close(unit=18)
      ...
      real*4 defthick2(50),defdens2(50),defvp2(50),defvs2(50)
      real*4 blat(100),blon(100),bdepth(30),bdVs(100,100,30) 
      real*4 minlat,minlon,maxlat,maxlon,latinc,mindepth,maxdepth     
      read(11,*) beglat,endlat,dlat, beglon,endlon,dlon
      read(22,*) beglat,endlat,dlat, beglon,endlon,dlon
c  11 is ab initio starting model
c  22 is starting model using result of previous inversion.  Need both
c  because Brandon's model is percent variation from a uniform velocity
c  mantle model, so need uniform mantle from 11, but need new crustal results
c  from 22
	nx = (endlon-beglon)/dlon + 1.01
	ny = (endlat-beglat)/dlat +1.01
         nxy =  nx * ny
      read(18,*) minlat,minlon,maxlat,maxlon,latinc,mindepth,maxdepth,
     1   ndepth,vs2vp,vs2dens
c these parameters describe range of body wave grid.  vs2vp and vs2dens describe
c ratio of p and density perturbations to shear wave perturbations (%) - 
c vs3vp = 1 keeps constant poisson's ratio
       nby = (maxlat-minlat)/latinc +1.01
       nbx = (maxlon-minlon)/latinc +1.01
       nbxy = nbx*nby
c  this routine for introducing perturbations to uniform mantle model assumes
c  that grid spacing same for mantle model and perturbation model. although 
c  there may be fewer points in perturbation model.  Also, same number of depths
c  for every lat,long point in Brandon's model.
       do j = 1, nbx
         do i = 1,nby
	   do k = 1, ndepth
	     read(18,*) blat(i),blon(j),bdepth(k),bdVs(i,j,k)
	   enddo
	 enddo
       enddo
	   ...
	   cumlayer = 0.0
	   do ii = 1, nlay
	     read(11,*) defthick(ii),defdens(ii),defvp(ii),defvs(ii)
	     read(22,*) defthick2(ii),defdens2(ii),defvp2(ii),defvs2(ii)
             dpth(ii) = cumlayer + 0.5*defthick(ii)
	     cumlayer = cumlayer + defthick(ii)
	   enddo
c  check whether within body wave grid
	   if  (((slon.ge.minlon).and.(slon.le.maxlon)).and.
     1	     ((slat.ge.minlat).and.(slat.le.maxlat))) then
	     i = (slat-minlat)/latinc +1.01
	     j = (slon-minlon)/latinc + 1.01
	     do ii = 1,nlay
c  check whether in crust or shallower (or deeper) than depth limits of body wave model
	       if ((dpth(ii).lt.mindepth).or.(dpth(ii).gt.maxdepth).or
     1	            .(defdens(ii).lt.3.2)) then
	         write(21,*) defthick2(ii),defdens2(ii),defvp2(ii),defvs2(ii)
	       else
c Find depth interval, then linearly interpolate perturbation value to middle of layer
c  If top or bottom, interpolate to zero at mindepth or maxdepth
                  if (dpth(ii).lt.bdepth(1)) then
		    prtrbfrac = 0.01*bdVs(i,j,1)
     1                   *(dpth(ii)-mindepth)/(bdepth(1)-mindepth)
                  endif
		  if (dpth(ii).gt.bdepth(ndepth)) then
		    prtrbfrac = 0.01*bdVs(i,j,ndepth)
     1                *(maxdepth - dpth(ii))/(maxdepth -bdepth(ndepth))
                  endif
		  if ((dpth(ii).ge.bdepth(1)).
     1                  and.(dpth(ii).le.bdepth(ndepth))) then
                    do jj = 1, ndepth-1
		      if ((dpth(ii).ge.bdepth(jj)).
     1                  and.(dpth(ii).lt.bdepth(jj+1))) inddpth = jj
		    enddo
		    prtrbfrac = 0.01*(bdVs(i,j,inddpth) + 
     1		    (bdVs(i,j,inddpth+1)-bdVs(i,j,inddpth)) *
     2		    (dpth(ii)-bdepth(inddpth)) /
     3		    (bdepth(inddpth+1)-bdepth(inddpth)))
                  endif
		  defvs(ii) = (1.0+prtrbfrac)*defvs(ii)
		  defvp(ii) = (1.0+vs2vp*prtrbfrac)*defvp(ii)
		  defdens(ii)= (1.0+vs2dens*prtrbfrac)*defdens(ii)
c		  write(*,*) slat,slon,i,j,prtrbfrac
c		  write(*,*) 
c     1           dpth(ii),defthick(ii),defdens(ii),defvp(ii),defvs(ii)
		  write(21,*) defthick(ii),defdens(ii),defvp(ii),defvs(ii)
		endif
              enddo
	   else
	     do ii = 1, nlay
