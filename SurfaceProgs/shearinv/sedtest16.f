c  shearz16.f differs from shearz6.f in outputting complete covariance matrix 
c  for each inversion and optionally inputting covariance matrix from previous
c  inversion to be used as the a priori model covariance matrix  

c  shearz6.f differs from shearz5.f in correcting only for effects of negative
c  lobes that are deeper than target depth - one sided correction  Also includes effect of density
c  changes, with some proportionality to S wave changes assumed.

c  shearz5.f differs from shearz4.f in correcting for effects of negative lobes of the resolution 
c  matrix vectors.  
c  THIS APPROACH DOESN'T HELP!

c  shearz4.f differs from shearz3.f in outputting complete resolution matrix for each vertical 
c  inversion

c  shearz3.f differs from shearz2.f in that it finds optimum smoothing parameter to minimize 
c  off-diagonal elements of a posteriori covariance matrix


c shear wave velocity. modified/written May 3, 2001 by DSW.
c This version do a group of inverison for shear velocity in each grid (0.2 x 0.2)


c This inversion uses matrix inversion routines from
c Don Forsyth and Aibing Li, simannerr42.f
...
********************************Modify here**************************
c nit: number of ieration 
c partial devirative of phase velocity with respect to shear velocity 
c slightly depends on shear velocity, in other word, this inverse problem
c is linear like, so solution will converge very quickly. (nit = 2)  
c nit=1, nit=2 show a little difference (a little bit smaller standard deviation with nit=2)
c nit=2, nit=6 almost no difference

c For linear problem, no iteration is need and inverse result doesn't 
c rely on the starting model  
c For nonlinear problem, approximate the problem using linear equations, 
c and then solve for a convergent solution based on iteration to satisfy 
c the minimum regularization. In this case, the solution does depend on
c the starting model, so an appropriate starting model will be greatly appreciated.
 
      parameter (npermax=30,maxlay=100,maxgrid=20000)  
*********************************************************************                                
...
c      real*4    oscil2(maxlay,maxlay),oscil(maxlay,maxlay)
      ...
      real*4    sumpredvel(npermax)
      real*4    remlon(maxgrid),remlat(maxgrid)
      real*4    rempred(npermax,maxgrid)
...
c  smoother model or to just stop at data inversion
c
...
c  topd and bottomd control depth range in which partial derivatives are output  
	...
	nlat = (endlat-beglat)/dlat +1.01
        ...
        nxy = nlat*nlon
c  TESTING ONLY, limit nxy
c        nxy = 2

	do i = 1,nper
	  read(*,*) phseout(i)
	  open(60+i, file = phseout(i))
...
c nparam: number of layers which model needs to be resolved
c shear velocity can be resolved by data       
        np = nlay
        nparam = np  
...
c  change sensitivity to shear velocity change to density units
	    if (stbeta(ii).ne.0.0) then
	      densrat(ii) = rho2s*dens(ii)/stbeta(ii)
	    else
	      densrat(ii) = 0.0
	    endif
         enddo
...
c start loop for iterations
         do kk=1,nit
             if (kk.eq.1) then
            
               do i=1,np
                  origmod(i)=stbeta(i)
                  crrntmodb(i)=stbeta(i)
		  crrntmoda(i) = stalpha(i)
		  crrntmodr(i) = dens(i)
               enddo
...
c read in observed data                                                                 !
	     do i=1,nobs                                                                     !
	        read(12,*) ccobs(i),ccobstd(i)                                               !
c multiply standard deviations by constant to compensate for usual underestimate                                                   !
	        stddevdata(i)=stdmult*ccobstd(i)                                             !
	     enddo                                                                           ! 
*****************************************************
             if (inxy.ne.1434) go to 4000
                                                                                        ! 
**************modify here****************************                                   ! 
...
c*********************************************************************
     ...
     r     0 0 1'           
         write(30,*)'(F6.2,3F8.4)'
         do ii=1,np
            write(30,210) zero,crrntmodr(ii),crrntmoda(ii),crrntmodb(ii)
            write(30,210) thick(ii),crrntmodr(ii),crrntmoda(ii),
     1              crrntmodb(ii)
         enddo
...
c read in predicted data
c here c(w) obtained from s_saito which also calculates 
c partial derivatives,dcb,dca
c from a starting layered model (e.g. ak135 or PREM)
c for Rayleigh waves dcb=combo of P and SV: 
c e.g., dcb(z)=dcb + p2s*dca                              
...
c  assume P velocity changes are proportional to shear velocity with ratio p2s	   
            g(i,j)= ((dcb(j,i) + p2s*dca(j,i)+densrat(j)*dcr(j,i))
     r          *thick(j))/stddevdata(i)

	   enddo
	enddo

         close (16)
c	 write(*,*) 'data', lon,lat
       do i=1,nobs
	  d(i)=(ccobs(i)-ccpred(i))/stddevdata(i)
       enddo		
c        write(*,*) (d(i), i = 1, nobs)
...
c  Update current model  - check for absurd results of low shear velocity

         do ii = 1, np
          crrntmodb(ii) = crrntmodb(ii) + change(ii)
	  if ((ii.gt.1).and.(crrntmodb(ii).lt.0.2)) crrntmodb(ii) = 0.2	  
         enddo
c  Check to make sure shear velocity in water layer stays at zero
         if (abs(origmod(1)).lt.0.1) crrntmodb(1) = 0.0
        do j = 1, np
          stddev(j) = sqrt(gtginv(j,j))
        enddo

c calculate new layered earth model for input to saito.f to get
c residuals for this inversion
c write out velocities in saito.f format
	...
	do ii=1,np
	   crrntmoda(ii) = crrntmoda(ii) + p2s*change(ii)
	   crrntmodr(ii) = crrntmodr(ii) + densrat(ii)*change(ii)
	   write(30,210) zero,dens(ii),crrntmoda(ii),crrntmodb(ii) 
	   write(30,210) thick(ii),crrntmodr(ii),crrntmoda(ii),crrntmodb(ii)  
	enddo
...
c calculate residuals for the new shear wave vel. profile
c must calculate new phase velocities from saito.f with this
c beta(z) profile and get difference from data	

	call s_saito
        
      open(16, file = 'DERIV.DATA')

	 write(18,*)'T obs pred  data stddv  residuals  norm residuals'
         sumresid  = 0.0
        do i=1,nobs
            read(16,*) xxd0
            read(16,*) xxd1
            read(16,*) xxd2,xxd3,xxp,ccpredout(i)
c	    write (*,*) "iter ", i,xxp,ccpredout(i)
            read(16,*) xxd4
	...
	enddo
	    ...
	    if (kk.eq.nit) sumnormmsft2 = sumnormmsft2 + standardresid**2
	    write(18,*)                          
	    write(18,*) '   RMS    stddv(divided by degree of freedom)'     !!!! for model
	    write(18,*) RMS,  standardresid 
	    write(18,*)  
190     format(F6.2, 2F12.4, 3F14.4) 
      
      close(unit = 16)  
c  Correct for effect of negative lobes in resolution matrix 
         if (kk.lt.nit) then
	   do i = 1, np
	      correct(i) = 0.0
	   enddo
	   do j = 1, np
c	     do j = 1,np
c  make this correction one-sided - correct only if deeper
c  i.e. effect of change in layer j on deeper layers is corrected
             do i = j,np
	       if (resmatx(i,j).lt.0.0) then
	         correct(i) = correct(i) + resmatx(i,j)*change(j)/resmatx(j,j)
	       endif
	     enddo
	   enddo
	   do i = 1,np
	     write(18,*) i, correct(i)
c	     crrntmodb(i) = crrntmodb(i) - correct(i)
c	     crrntmoda(i) = crrntmoda(i)- p2s*correct(i)
c  Try correcting from starting model 
	     crrntmodb(i) = stbeta(i) - rmvnegfac*correct(i)
	     crrntmoda(i) = stalpha(i)- rmvnegfac*p2s*correct(i)
	     crrntmodr(i) = dens(i) - rmvnegfac*densrat(i)*correct(i)
           enddo
	 endif   
      
                 
c      close(unit = 19)       
      enddo  ! close loop from iterations

	do ii = 1, np
	   write(90,290) thick(ii),crrntmodr(ii),crrntmoda(ii),crrntmodb(ii)  
	enddo	    
290     format (f6.3,2x,f5.3,2x,f6.3,2x,f6.3)
        write (50,*) inxy, lon,lat
        do i = 1,np
	  do j= 1,i
...
c	  write(60+i,*) lon,lat,ccpredout(i)
c	  sumpredvel(i) = sumpredvel(i) + ccpredout(i)
c	enddo

        do i = 1, nper
	  rempred(i,inxy) = ccpredout(i)
	  sumpredvel(i) = sumpredvel(i) + ccpredout(i)
	enddo
4000    continue
	enddo  ! close all grid points
	
	avgnormmsft2 = sumnormmsft2/nxy 
	write(50,*) avgnormmsft2, ' avgnormmsft2'
	write(18,*) avgnormmsft2, ' avgnormmsft2'

	do i = 1, nper
          avgvel = sumpredvel(i)/nxy
	  write(60+i,*) avgvel
	  do inxy = 1, nxy
	    write(60+i,*) remlon(inxy),remlat(inxy),rempred(i,inxy)
	  enddo
	  close(60+i)
	enddo
