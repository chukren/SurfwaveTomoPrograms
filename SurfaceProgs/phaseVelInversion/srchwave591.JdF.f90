...
!  srchwave591.phc.kres.f  differs from 587 in using uniform velocity start and no 
!       station phase corrections  -  mostly use for finding problems in data
!                                  srchwave587.kern.br.d.phc.kres.f  
!  2-D grid for isotropic terms plus uniform 2theta anisotropy 
!
!  587 differs from 586 by correcting for first order effect of lateral velocity changes
...
!      character*100 foutput,fsummary
      ...
      character*130 fn(maxevnts,maxnsta)
...
!c formula from Ying Zhou et al., 2004, GJI 3-D sensitivity kernels for surface-wave observables
...
!     1        phsens(ixkern,iykern),amsens(ixkern,iykern)
	  enddo
	enddo
...
! assign velocities to each grid node apriori - DISABLED IN 591
...
!C        unifvel = preunifvel
!C	nodevel(i) = nodevel(i) + unifvel - preunifvel
...
!  make covinv damping much tighter for grid search than for simulated annealing so that linearized
!  step following grid search for wave parameters won't go far astray.  
...
!       write(17,*) iev, amplarge, amprms(iev,ifreq)
...
!   calculate the sensitivity kernel for each possible angle between -40 to 40
...
!       ystatemp = 
!     1  -xsta(iev,iref(iev))*sin(staziminc1)
!     1 + ysta(iev,iref(iev))*cos(staziminc1)     
...
!       ystatemp = 
!     1  -xsta(iev,iref(iev))*sin(staziminc2)
!     1 + ysta(iev,iref(iev))*cos(staziminc2)     
...
!       ystatemp = 
!     1  -xsta(iev,ista)*sin(staziminc1)
!     1 + ysta(iev,ista)*cos(staziminc1)     
...
!       ystatemp = 
!     1  -xsta(iev,ista)*sin(staziminc2(iev))
!     1 + ysta(iev,ista)*cos(staziminc2(iev))     
