...
c  srchwave591.phc.kres.f  differs from 587 in using uniform velocity start and no 
c       station phase corrections  -  mostly use for finding problems in data
c                                  srchwave587.kern.br.d.phc.kres.f  
c  2-D grid for isotropic terms plus uniform 2theta anisotropy 
c
c  587 differs from 586 by correcting for first order effect of lateral velocity changes
...
c assign velocities to each grid node apriori - DISABLED IN 591

c         call assignstrt(startvel,nodevel,preunifvel,nnodes,
c     1                           ncol,dxnode,dynode)
...
CC        unifvel = preunifvel
CC	nodevel(i) = nodevel(i) + unifvel - preunifvel
...
c	call updateapri(fendvel,unifvel,preunifvel,dampvel,nnodes)
