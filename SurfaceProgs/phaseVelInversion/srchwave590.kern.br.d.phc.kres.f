...
c  srchwave590.kern.br.d.phc.kres.f differs from 589 in solving for aniso on different
c  WARNING  - this involved major changes from other versions, including subroutine calls
c  etc.  definitely not backward compatible   WARNING

c  scale than isotropic lateral variations - different grid
...
c  This version reads in non-uniform starting model
      ...
      parameter (maxnfreq=1, maxnsta=300, maxpts=20000, nparam = 4165,
     1           maxnobs = 24000, maxnodes = 2000, maxevnts = 400,
     1           maxnxints = 401, maxnyints = 401,ndeg = 81 )
      ...
      real*4 nodelat2(maxnodes),nodelon2(maxnodes)
     ...
     1       dampper(maxnsta,ndeg)
      ...
      real*4 vchangecs(maxnodes),vchangesn(maxnodes)
      
      real*4 b(maxnodes,maxnodes),btb(maxnodes,maxnodes)
      real*4 b2(maxnodes,maxnodes),btb2(maxnodes,maxnodes)
      ...
      integer*4 nnodes, nnodes2, nevents, ntyp1                                
      ...
      real*8 sensitivity2(maxnxints,maxnyints)
      real*8 ampsens2(maxnxints,maxnyints)
...
c      character*100 foutput,fsummary
      ...
      common /nodepts/ nodelat,nodelon,nodelat2,nodelon2,nnodes,nnodes2
     ...
     1 ityp1sta,ntyp1,iref,nsta,iev,naddat,ifreq                 
     ...
     2   nevents,idnode,ideg1,ideg2,nobs,i6,iarea
       ...
       common /gengrid/ boxlat,boxlon
       
       common /update2/ covar,vchange,vchangecs,vchangesn,stddev
...
c202         format(a97, a4)
202         format(a99, a4)
c changed format a97 to a99 to get the station name
...
c  first set up nodes for isotropic terms
      call genreggrid(finvrsnodes,nodelat,nodelon,nnodes,ncol,dxnd,dynd)
       dxnode = abs(dxnd)
       dynode = abs(dynd) 
c  then set up nodes for azimuthal anisotropic terms 
       call genreggrid(finvrsnodes2,nodelat2,nodelon2,nnodes2,
     1      ncol2,dxnd,dynd)
       dxnode2 = abs(dxnd)
       dynode2 = abs(dynd)  
	 ...
	 enddo	
	  ...
	  do iykern = 1,nykern
	    ...
	    phsens(ixkern,iykern) = 0.0
            ...
            sensitivity2(ixkern,iykern) = 0.
                ampsens2(ixkern,iykern) = 0.
	  enddo
	  enddo
...
cc formula from Ying Zhou et al., 2004, GJI 3-D sensitivity kernels for surface-wave observables
	...
	amsens(ixkern,iykern) = amsens(ixkern,iykern) +
	 ...
	 enddo
	enddo	     
c  now take these raw sensitivity kernels and find net effect for isotropic node velocities using
...
c  renormalize sensitivity kernels
	sensnorm =  (dxnode*dynode)/(dxkern*dykern)
	  ...
	  enddo
	  enddo
c  now take these raw sensitivity kernels and find net effect for anisotropic node velocities using
c  linear interpolation between nodes, essentially convolving interpolater with raw kernels - use 
c  isotropic kernels as crude approximation for sensitivity to anisotropic terms even though this is
c  clearly not strictly correct
        ixoff = dxnode2/dxkern
	iyoff = dynode2/dykern
	do ixkern = 1, nxkern
	  ixxbeg = ixkern - ixoff
	  ixxend = ixkern + ixoff
	  if (ixxbeg.lt.1) ixxbeg = 1
	  ...
	  do iykern = 1,nykern	
	    ...
	    do ix = ixxbeg, ixxend
	      ...
	      do iy = iyybeg,iyyend
	        ykk = (iy-1)*dykern + ybegkern
		wgtkern = (1.-abs(xk-xkk)/dxnode2)*(1.-abs(yk-ykk)/dynode2)
		sensitivity2(ixkern,iykern) = sensitivity2(ixkern,iykern) 
     1                            + phsens(ix,iy)*wgtkern
     		ampsens2(ixkern,iykern) = ampsens2(ixkern,iykern)
     1                            + amsens(ix,iy)*wgtkern
		wgtsum = wgtsum+wgtkern
	      enddo
	    enddo
	    sensitivity2(ixkern,iykern) = sensitivity2(ixkern,iykern)/wgtsum
	    ampsens2(ixkern,iykern) = ampsens2(ixkern,iykern)/wgtsum
c	write(14,*) xk,yk,sensitivity(ixkern,iykern),ampsens(ixkern,iykern),
c     1        phsens(ixkern,iykern),amsens(ixkern,iykern)
	  enddo
	enddo
c  renormalize sensitivity kernels
	sensnorm =  (dxnode2*dynode2)/(dxkern*dykern)
          do ixkern = 1,nxkern
	  do iykern = 1,nykern
	     sensitivity2(ixkern,iykern) = sensitivity2(ixkern,iykern)*sensnorm
	     ampsens2(ixkern,iykern)= ampsens2(ixkern,iykern)*sensnorm
	  enddo
	  enddo
...
c assign velocities to each grid node apriori
         ...
         call assignstrt(startvel,nodevel,preunifvel,
     1                  nnodes,ncol,dxnode,dynode)
         call assignstrt2(startaniso,nodecos2,nodesin2,
     1            nnodes2,dxnode2,dynode2)
...
c	read(20,*) iarea
c
c  In version 590, iarea = nnodes2
c	 iarea = 1
        iarea = nnodes2
	do i=1, iarea
...
c  stations and attenuation factor gamma
	...
	kj2 = nnodes2/ncol2
        i6 = 6*nevents
	if (np.gt.nparam) write (*,*) 'WARNING  too many parameters'
c   ***************************************
...
c          origmod(ipp) = 0.0
c          origmod(ippp) = 0.0
          origmod(ipp) = nodecos2(i)
	  origmod(ippp) = nodesin2(i)
...
c  of relative position
     ...
     1                                       ,delta,bazimnd(inode))
      ...
      enddo
      do inode = 1, nnodes2
        call disthead(applat,applon,nodelat2(inode),nodelon2(inode)
     1                                       ,delta,tazim)
        call disthead(nodelat2(inode),nodelon2(inode),applat,applon
     1                                    ,delta,bazimnd2(inode))
        xnode2(iev,inode) = appcirc*delta - stadist(iev,iref(iev))
        azidiff = tazimref - tazim
        if (azidiff.gt.180.) azidiff = azidiff -360.
        if (azidiff.lt.-180.) azidiff = azidiff +360.
        ynode2(iev,inode) = appcirc*sin(delta*convdeg)*azidiff
      enddo
...
c  construct form of a priori smooothness criterion matrix for anisotropy 
c  parameters  (minimum curvature)  Same form for cos2theta and sin2theta
c  terms
...
c  initialize
      do ismth = 1, nnodes2
        do jsmth = 1, nnodes2
	  b2(ismth,jsmth) = 0.0
	  btb2(ismth,jsmth) = 0.0
	enddo
      enddo
c  curvature terms in y-direction - but skip at edges where undefined
      do ismth = 1,ncol2
        do jsmth = 2, kj2-1
	  ism = (ismth-1)*kj2 + jsmth
	  b2(ism,ism) = 2.0 +b2(ism,ism)
	  b2(ism,ism-1) = -1.0+ b2(ism,ism-1)
	  b2(ism,ism+1) = -1.0+ b2(ism,ism+1)
	enddo
      enddo
c  curvature terms in x-direction
	  ...
	  ism = (ismth-1)*kj2 + jsmth 
	  b2(ism,ism) = 2.0 +b2(ism,ism)
	  b2(ism,ism-kj) = -1.0 + b2(ism,ism-kj)
	  b2(ism,ism+kj) = -1.0 + b2(ism,ism+kj)
	enddo
      enddo
c  now construct BtransposeB	
        do j = 1, nnodes2
          do jj = 1,j
            btb2(jj,j) = 0.0
            do i = 1, nnodes2
              btb2(jj,j)= btb2(jj,j) + b2(i,jj)*b2(i,j)
            enddo
            btb2(j,jj) = btb2(jj,j)
          enddo
        enddo
...
c  calculate current apparent velocity at each node - assuming that result will
c  be linear summation of effects due to variation in isotropic and anisotropic 
c  terms 
...
c  loop over stations, generating sensitivity kernels for each
	    ...
	    ampwgtnode1(ista,ii,ideg) = 0.0
          enddo
          do ii = 1, nnodes2
	      wgtnode2(ista,ii,ideg) = 0.0
	    ampwgtnode2(ista,ii,ideg) = 0.0
          enddo
...
c        write(*,*) 'isotropic',xnode(iev,ii),ynode(iev,ii)
c	write (*,*) xstanode,ystanode, xstatemp,ystatemp

	    write(*,*) 'ixindex,iyindex,idnum,ista,ideg', 
     1            ixindex,iyindex,idnum(iev),ista,ideg
	     ...
	     ampwgtnode1(ista,ii,ideg) = 0.  
          ...
          do ii = 1,nnodes2
           ...
           xnodetemp = 
     1      xnode2(iev,ii)*cs1z
     1      + ynode2(iev,ii)*sn1z               
     
           ynodetemp = 
     1      -xnode2(iev,ii)*sn1z
     1      + ynode2(iev,ii)*cs1z     
...
c  dxkern << smoothing length
     ...
     1    .or. iyindex .lt.1 .or. iyindex .gt. nykern) then 
	    ...
	    write(*,*) 'ixindex,iyindex,idnum,ista,ideg', 
     1            ixindex,iyindex,idnum(iev),ista,ideg
c        write(*,*) 'anisotropic', xnode2(iev,ii),ynode2(iev,ii)
c	write (*,*) xstanode,ystanode,xstatemp, ystatemp
c	write(*,*) xsta(iev,ista),ysta(iev,ista)
	     wgtnode2(ista,ii,ideg)    = 0.
	     ampwgtnode2(ista,ii,ideg) = 0.  

 	   else

             wgtnode2(ista,ii,ideg) = sensitivity2(ixindex,iyindex)
             ampwgtnode2(ista,ii,ideg) =     ampsens2(ixindex,iyindex)
           endif 
	   else 
	     wgtnode2(ista,ii,ideg)    = 0.
	     ampwgtnode2(ista,ii,ideg) = 0.  
...
c   and later old version commented out
          do inode = 1, nnodes
...
c     1           *(appvel(inode)-unifvel)/unifvel
     ...
     3           /(appvel(inode)/unifvel)
          ...
          do inode = 1, nnodes2
          dphase(ista,ideg) = dphase(ista,ideg) 
     1  + (1.0/twopi)*wgtnode2(ista,inode,ideg)
     1           *(appvel2(inode))/unifvel
          enddo
...
c     1   (appvel(inode)-unifvel)/unifvel
     ...
     3           /(appvel(inode)/unifvel)
	  ...
	  do inode =1, nnodes2
          dampper(ista,ideg) = dampper(ista,ideg) 
     1  +ampwgtnode2(ista,inode,ideg)*
     2   (appvel2(inode))/unifvel
          enddo
...
c  replace unifvel in following four partials with runifvel because
c  have done nonlinear version in dphase in versions 586 and later
c  
          do ii = 1, nnodes
        runifvel = 1.0/(-unifvel/(appvel(ii)**2) + 
     1              2.0*unifvel**2/(appvel(ii)**3))

       parph1v = (1.0/twopi)*wgtnode1(ista,ii,ideg1)/runifvel
     1         - (1.0/twopi)*wgtnode1(iref(iev),ii,ideg1)/runifvel

       parph2v =  (1.0/twopi)*wgtnode1(ista,ii,ideg2)/runifvel
     1          - (1.0/twopi)*wgtnode1(iref(iev),ii,ideg2)/runifvel

       paramp1v = startamp1(iev)*ampwgtnode1(ista,ii,ideg1)/runifvel 
       paramp2v = startamp2(iev)*ampwgtnode1(ista,ii,ideg2)/runifvel


c  partial derivatives with respect to velocity
     ...
     1                         wgtnode2(ista,ii,ideg1)/unifvel
     1          - (1.0/twopi)*cos2node(ii,ideg1)*
     1                         wgtnode2(iref(iev),ii,ideg1)/unifvel

       parph2cs = (1.0/twopi)*cos2node(ii,ideg2)*
     1                         wgtnode2(ista,ii,ideg2)/unifvel
     1          - (1.0/twopi)*cos2node(ii,ideg2)*
     1                         wgtnode2(iref(iev),ii,ideg2)/unifvel
     ...
     1                            ampwgtnode2(ista,ii,ideg1)/unifvel

       paramp2cs  = startamp2(iev)*cos2node(ii,ideg2)*
     1                            ampwgtnode2(ista,ii,ideg2)/unifvel
...
c  partial derivatives with respect to cos2theta &sin2theta
...
c increase damping for wave parameters
        ...
        enddo
c  ******************************
c   add smoothness constraint (minimum curvature) for isotropic terms.  Find coefficient, smthco, 
...
c
	...
	do ismth = 2,nnodes2
	  do jsmth = 1, ismth-1
	    sumsmth2 = sumsmth2 + btb2(ismth,jsmth)**2
	    sumsmgg = sumsmgg + btb2(ismth,jsmth)*
...
c   Find maximum change to model (well, to isotropic velocity
...
c  the rank is not 2*n+2 because the velocity is not independent.do the if test in our case 
	      ...
	      if ((2*nsta(iev) - (rank2+rank3+rank4)/nevents - 6).lt.1.0) then
	      sigma2=sumsq/1.0
	      else
	      sigma2 = sumsq/(2*nsta(iev) -(rank2+rank3+rank4)/nevents - 6)
	      endif
...
c	    write(16,*) subgtginv(ii,jj)
        ...
        do ii = 1, nnodes
c
          write(10,*) ii, crrntmod(ii+i6),stddev(ii+i6),
     1          residdiag(ii+i6),nodelon(ii),nodelat(ii)
...
c     1              crrntmod(ii+i6),stddev(ii+i6),vchange(ii)
        enddo
        write(30,*) nnodes2
	write(10,*) 'anisotropic terms'
	write(11,*) 'anisotropic terms'
        do ii = 1, iarea
           iii= i6 + nnodes +ii
           iiii = iii+iarea
	   vchangecs(ii) = crrntmod(iii)-nodecos2(ii)
	   vchangesn(ii) = crrntmod(iiii) - nodesin2(ii)
           write(10,1277) crrntmod(iii),stddev(iii), residdiag(iii),
     1        crrntmod(iiii),stddev(iiii),residdiag(iiii),nodelon2(ii),
     2        nodelat2(ii)
           write(11,1277) crrntmod(iii),stddev(iii), residdiag(iii),
     1        crrntmod(iiii),stddev(iiii),residdiag(iiii),nodelon2(ii),
     2        nodelat2(ii)
           write(30,1276) nodelon2(ii),nodelat2(ii), 
     1        crrntmod(iii),stddev(iii), residdiag(iii),
     2        crrntmod(iiii),stddev(iiii), residdiag(iiii)
	enddo
1276    format(f9.3,f8.3,6(f8.4))
1277    format(6(f8.4),f9.3,f8.3)
        jcnt = 0
        do ii = 1,mxnsta
...
c	  write(20,*) nodelon(ii),nodelat(ii), residrow(ii)
	  write(21,*) nodelon(ii),nodelat(ii),residdiag(ii+i6)
	enddo
c  change predicted phase velocities from a priori model to new results on
c  same grid as a priori model
        
	call updateapri(fendvel,unifvel,preunifvel,dampvel)
	call updateapri2(finishaniso, dampaniso, i6)
...
14    continue
      ...
      PARAMETER (NMAX=6000,TINY=1.0e-20)
      INTEGER i,imax,j,k
...
c	write(*,*) 'nobs ', nobs, ' ifreq ', ifreq,' unifvel ',unifvel
	return
      ...
      subroutine genreggrid(freggrid,nodelat,nodelon,ntot,nxpt,dx,dy)
***************************************************************************
...
c  arrange points in standard order in single dimensioned arrays
  ...
  200  format(f7.3,f10.3)
      ...
      subroutine assignstrt(startvel,nodevel,preunifvel,
     1            ntot,nxpt,dxnode,dynode)
...
c	write(*,*) minlat,maxlat,minlon,maxlon
      ...
      subroutine assignstrt2(startaniso,nodecs,nodesn,
     1            ntot,dxnode2,dynode2)
      parameter (maxnodes = 2000, maxstart = 200)
      character*70 startaniso
      real*4 boxlat(4), boxlon(4)
      real*4 nodelat(maxnodes),nodelon(maxnodes)
      real*4 nodecs(maxnodes),nodesn(maxnodes)
      real*4 nodelat2(maxnodes),nodelon2(maxnodes)
      real*4 latmin,latmax,lonmin,lonmax
      real*4 prdlon2(maxstart),prdlat2(maxstart)
      real*4 prdsn(maxstart,maxstart),prdcs(maxstart,maxstart)
      integer*4 nnodes,nnodes2
      common /gengrid/ boxlat,boxlon
      common /nodepts/ nodelat,nodelon,nodelat2,nodelon2,nnodes,nnodes2

      common /updatest2/ prdlon2,prdlat2,prdcs,prdsn,dylat2,dxlon2,
     1    beglat2,endlat2,dlat2,beglon2,endlon2,dlon2,nlat2,nlon2
     
      pi = 3.1415928
      convdeg = 3.1415928/180.

      deg2km = 111.194
      open(61, file = startaniso)
c  assigns start values for anisotropic node parameters based on a priori predictions of
c  starting model, previously generated.  Important that a priori grid extend
c  at least as far as model grid and that it is nearly as closely or more closely
c  spaced than model grid. Uses linear weighting between nodes to generate average value for
c  starting values.  A priori grid does not have same spacing and is assumed to 
c  be regularly spaced on Mercator projection, increasing in both lon and lat
c  
      read(61,*) beglat2,endlat2,dlat2,beglon2,endlon2,dlon2
      nlat2 = (endlat2-beglat2)/dlat2 +1.01
      nlon2 = (endlon2-beglon2)/dlon2 + 1.01
c  if izero = 0 , then there is no a priori model for aniso - generate grid and
c  assign values of zero.
      read(61,*) izero
      if (izero.eq.0) then
        do ilat = 1, nlat2
          prdlat2(ilat) = beglat2 + (ilat-1)*dlat2
          do ilon = 1, nlon2
	    prdlon2(ilon) = beglon2 + (ilon-1)*dlon2
	    prdcs(ilon,ilat) = 0.0
	    prdsn(ilon,ilat) = 0.0
	  enddo
        enddo
...
c  before incrementing in latitude
        do ilat = 1, nlat2
          do ilon = 1, nlon2
	    read(61,*) prdlon2(ilon),prdlat2(ilat),prdcs(ilon,ilat),prdsn(ilon,ilat)
	  enddo
        enddo
      endif
c  convert spacing of model back to degrees
      dylat2 = dynode2/deg2km
      dxlon2 = dxnode2/deg2km
      do i = 1, ntot
        sclfac = cos(convdeg*nodelat2(i))
	dxxlon = dxlon2/sclfac
        latmin = nodelat2(i) - dylat2
	latmax = nodelat2(i) + dylat2
	lonmin = nodelon2(i) - dxxlon
	lonmax = nodelon2(i) + dxxlon
	minlat = (latmin - beglat2)/dlat2 +1
	if (minlat.lt.1) minlat = 1
	maxlat = (latmax-beglat2)/dlat2 +1
	if (maxlat.gt.nlat2) maxlat = nlat2
	minlon = (lonmin-beglon2)/dlon2 +1
	if (minlon.lt.1) minlon = 1
	maxlon = (lonmax-beglon2)/dlon2 + 1
	if (maxlon.gt.nlon2) maxlon = nlon2
	wgtsum = 0.0
	sumvelcs = 0.0
	sumvelsn = 0.0
	if ((minlat.gt.nlat2).or.(maxlat.lt.1).or.
     1             (minlon.gt.nlon2).or.(maxlon.lt.1)) then
           write (*,*) 
     1   'PROBLEM. Node outside region of starting predictions '
	   write (*,*) i, nodelat2(i),nodelon2(i)
	endif
	do ii = minlon,maxlon
	  do jj = minlat,maxlat
	    wgt = (1.-abs(nodelat2(i)-prdlat2(jj))/dylat2)
     1             *(1.-abs(nodelon2(i)-prdlon2(ii))/dxxlon)
            sumvelcs = sumvelcs + wgt*prdcs(ii,jj)
            sumvelsn = sumvelsn + wgt*prdsn(ii,jj)
	    wgtsum = wgtsum + wgt
	  enddo
	enddo
	nodecs(i) = sumvelcs/wgtsum
	nodesn(i) = sumvelsn/wgtsum
c	write(*,*) i, nodevel(i),nodelat(i),nodelon(i)
c	write(*,*) minlat,maxlat,minlon,maxlon
      enddo
      return
      ...
      subroutine updateapri(fendvel,unifvel,preunifvel,dampvel)
...
c  which nodes are within interpolation range for every point
      parameter (maxnodes = 2000, maxstart = 200, nparam = 4165)
      character*70 fendvel
      real*4 boxlat(4), boxlon(4)
      real*4 nodelat(maxnodes),nodelon(maxnodes),nodevel(maxnodes)
      real*4 nodelat2(maxnodes),nodelon2(maxnodes)
...
c	  write(*,*) ii,jj,nwgt,(iwgt(ij),ij=1,nwgt),prdlon(ii),prdlat(jj)
	...
	enddo
      ...
      subroutine updateapri2(finishaniso,dampaniso,i6)
c  Take inversion solution of anisotropic terms and go through
c  linear interpolation to detailed, a priori starting model. Also, generate 
c  standard deviation of estimate
c
c  Slow process because nodes not on regular grid of lat&lon - have to search for
c  which nodes are within interpolation range for every point
      ...
      real*4 prdsn(maxstart,maxstart),prdcs(maxstart,maxstart)
      real*4 vchangecs(maxnodes),vchangesn(maxnodes)
      real*4 newvelcs(maxstart,maxstart),stddevcs(maxstart,maxstart)
      real*4 newvelsn(maxstart,maxstart),stddevsn(maxstart,maxstart)
      real*4 stddev(nparam)
      integer*4 nnodes,nnodes2
      integer iwgt(maxnodes)
      common /gengrid/ boxlat,boxlon
     ...
     1    beglat2,endlat2,dlat2,beglon2,endlon2,dlon2,nlat2,nlon2
      ...
      open (70, file = finishaniso)
      write(70,*) beglat2,endlat2,dlat2,beglon2,endlon2,dlon2
      do jj = 1,nlat2
c  to correct for points on mercator grid being closer together away from equator
        sclfac = cos(prdlat2(jj)*convdeg)
        do ii = 1,nlon2
	  wgtsum = 0.0
	  cs2sum = 0.0
	  sn2sum = 0.0
	  ...
	  nwgt = 0
	  do kk = 1,nnodes2
	    xsep = abs(prdlon2(ii)-nodelon2(kk))*sclfac
	    if ((xsep.le.dxlon2).and.
     1          (abs(prdlat2(jj)-nodelat2(kk)).le.dylat2))  then
              wgt(kk) = (1.- abs(prdlat2(jj)-nodelat2(kk))/dylat2)
     1                       *(1.- xsep/dxlon2)
              nwgt = nwgt+1
c              iwgt(nwgt) = kk
	      iii = i6+nnodes+kk
	      iiii = iii+nnodes2
              wgtsum = wgtsum + wgt(kk)
	      cs2sum = cs2sum + wgt(kk)*vchangecs(kk)
c  CAUTION  next lines presume iarea = nnodes2
	      sumstdevcs = sumstdevcs + wgt(kk)*stddev(iii)
	      sn2sum = sn2sum + wgt(kk)*vchangesn(kk)
	      sumstdevsn = sumstdevsn + wgt(kk)*stddev(iiii)
	    endif
	  enddo
	  if (nwgt.gt.0) then
      	    newvelcs(ii,jj)=prdcs(ii,jj)+cs2sum/wgtsum
	    stddevcs(ii,jj) = sumstdevcs/wgtsum
	    newvelsn(ii,jj)=prdsn(ii,jj)+sn2sum/wgtsum
	    stddevsn(ii,jj) = sumstdevsn/wgtsum
...
c	    stddevap(ii,jj) = sqrt(sumcov)/wgtsum
	     ...
	     newvelcs(ii,jj) = prdcs(ii,jj)
	     stddevcs(ii,jj) = dampaniso
	     newvelsn(ii,jj) = prdsn(ii,jj)
	     stddevsn(ii,jj) = dampaniso
	   endif
	enddo
      enddo
      do jj = 1,nlat2
        do ii = 1,nlon2
	  write(70,*) prdlon2(ii),prdlat2(jj),newvelcs(ii,jj),stddevcs(ii,jj),
     1        newvelsn(ii,jj),stddevsn(ii,jj) 
	enddo
      enddo
